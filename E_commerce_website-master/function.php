...
function anasayfa_alisveris()
{
    ...
    $con = mysqli_connect("localhost", "root", "", "eticaret");
    $al_kategori = " SELECT * FROM urunler Order By urunler_id DESC LIMIT 30";
    $sonuc = mysqli_query($con, $al_kategori);
    while ($cekilen_veri = mysqli_fetch_array($sonuc)) {
...
function oturum_secenek()
{
    if (isset($_SESSION["userid"])) {
        ...
        echo "   <a class=\"top_links\" href=\"hesap.php\">Giriş Yap/Kaydol </a> ";
    }
}
     ...
     <div class='cop_kova'><a href='shop.php?&sil=$urunler_id'><i style='color: #2971f5;max-width:15px;height:20px;' class=\"fa fa-trash-o\"></i></a></div>
        ...
        if (isset($_SESSION["sepet"])) {
            ...
            <div class=\"cart_button view_cart\">
                                    <a href='cart.php'>Sepeti Görüntüle ve Ödemeye Git</a>
                                </div>
            ";
        }
    }
    ...
    if (isset($_POST['ode'])) {
        foreach ($_SESSION["sepet"] as $urun) {
            ...
            $sql = "INSERT INTO sepet(user_id,urun_id,urun_title,urun_fiyat,urun_resim,urun_beden,urun_adet)
                VALUE('$userid','$urunler_id','$urun_title','$urun_fiyat','$urun_resim','$urun_adet')";
        ...
        if (isset($_POST['ekle_urun'])) {
            ...
            if (empty($title) || empty($parent_altkategori_id)) {
                ...
                $sql = "INSERT INTO urunler(parent_altkategori_id,urun_kod,urunler_title,urunler_desc,urunler_fiyat,	
                    urunler_resim,urunler_resim_1,urunler_resim_2,urunler_adet)
            VALUE('$parent_altkategori_id','$urun_kodu','$title','$desc','$fiyat','$resim1_ad1','$resim1_ad2','$resim1_ad3','$adet')";
                $succes = mysqli_query($con, $sql);
    ...
    <form action='admin.php' method='post' enctype='multipart/form-data'>
                ...
                <div class=\"row\">
                    ...
                    <select name='kategori_id'>
                        <option value='1'>Çekirdek Kahveler(Kahveler)</option>
                        <option value='2'>Filtre Kahve(Kahveler)</option>
                        <option value='3'>Kapsül Kahve(Kahveler)</option>
                        <option value='4'>Türk Kahvesi(Kahveler)</option>
                        <option value='5'>Öğütülmüş Espresso(Kahveler)</option>
                        <option value='6'>Yöresel Kahve(Kahveler)</option>
                        <option value='7'>Filtre Kahve Makineleri(Kahve Makineleri)</option>
                        <option value='8'>Kahve Makineleri(Kahve Makineleri)</option>
                        <option value='9'>Espresso Makineleri(Kahve Makineleri)</option>
